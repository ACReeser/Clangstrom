...
public class GrenLaunch : NetworkBehaviour {
    ...
    public void CmdFire(Vector3 origin, Vector3 direction)
    {
        ...
        RpcFireLauncher(this.netId,
            ReloadTime,
            grenade.GetComponent<NetworkIdentity>().netId,
            origin,
            direction);
    }

    [ClientRpc]
    void RpcFireLauncher(
        NetworkInstanceId firer,
        float reloadTime,
        NetworkInstanceId grenadeID,
        Vector3 origin,
        Vector3 direction)
    {
        if (isLocalPlayer)
